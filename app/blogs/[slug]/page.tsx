...
import { BlogSchema, FAQSchema } from "@/app/components/SchemaGenerator";
...
const BlogPostPage = () => {
  ...
  const [blogFaqs, setBlogFaqs] = useState<FAQ[]>([]);
  ...
  const fetchBlogFAQs = async () => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL;
      const response = await axios.get(`${apiUrl}/api/faq/page/blogs/${slug}`);
      setBlogFaqs(response.data.faqs || []);
    } catch (error) {
  ...
  return (
    <>
      ...
      {/* FAQ Schema for blog-specific FAQs */}
      {blogFaqs.length > 0 && <FAQSchema faqs={blogFaqs} />}
